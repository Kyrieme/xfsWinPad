...
void StatusBar::Layout(int width, int dpi) {
    ...
    //   [6] 模型来源     [7] 定义提示（批次 106） [8] 静态检查（批次 87）
    ...
    const int defW  = S(250);   // 定义提示："<.dec 名> 第 N 行  <那一行原文>"
    ...
    // 右簇从右往左排，保证右缘严格单调递增
    parts_[8] = width;                       // 静态检查（最后段，右缘值被忽略）
    parts_[7] = parts_[8] - diagW;           // 定义提示
    parts_[6] = parts_[7] - defW;            // 模型来源
    ...
    ::SendMessageW(hwnd_, SB_SETPARTS, 9, (LPARAM)parts_);
}
...
// 批次 106：定义提示在 [7]，静态检查随之移到 [8]（见头文件里的索引说明）。
void StatusBar::SetDefinitionHint(const std::wstring& text) { SetPart(7, text); }
void StatusBar::SetDiagnostics(const std::wstring& text) { SetPart(8, text); }
